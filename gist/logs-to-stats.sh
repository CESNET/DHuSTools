...
if [ $? -ne 0 ]; then
	2>&1 printf "Reformatting users.pat\n"
	sed -i 's/\t/,\//' users.pat
	sed -i 's/\t/,/g' users.pat
	sed -i 's/$/,\//' users.pat
	sed -i 's/^/s\/\^/' users.pat
...
cat *.log | grep -E '(download.*by.*user.*completed)' | grep -v manifest.safe | grep -v xfdumanifest.xml | awk '{ print $10 " " $6 " " $15 }' | sed "s/['()]//g" | \
awk '{
	user=$1
